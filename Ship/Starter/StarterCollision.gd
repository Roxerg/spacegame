...
var back_thruster  :Node2D
var left_thruster  :Node2D
var right_thruster :Node2D
var front_thruster :Node2D
...
func _integrate_forces(state):
	...
	if not left and not right:
		if not linear_velocity.is_equal_approx(Vector2(0,0)):
			...
			if (linear_velocity+dir).length() < velocity_len*1.5:
				applied_force = dir*velocity_len*10
				right_thruster.visible=true
			elif (linear_velocity+dir.rotated(PI)).length() < velocity_len*1.5:
				applied_force = dir.rotated(PI)*velocity_len*10
				left_thruster.visible=true
